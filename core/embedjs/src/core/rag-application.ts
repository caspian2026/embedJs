...
import { cleanString, getUnique } from '@caspian2026/embedjs-utils';
...
export class RAGApplication {
    ...
    private async getModel(model: BaseModel | SIMPLE_MODELS | null): Promise<BaseModel> {
        if (typeof model === 'object') return model;
            ...
            const { OpenAi } = await import('@caspian2026/embedjs-openai').catch(() => {
                throw new Error('Package `@caspian2026/embedjs-openai` needs to be installed to use OpenAI models');
            });
